...
# --- Drawing Logic (Extracted to improve modularity) ---
def draw_all(hdc, full_rect, rectangles, countdowns, qrcodes, transparent_key):
    ...
    # Draw countdowns
    for idx, (_, cd) in enumerate(sorted(countdowns.items(), key=lambda x: x[1]["order"])):
...
class CloseWindowCommand(Command):
    def execute(self, overlay_manager, args, reply_queue):
        ...
        if window_id:
            ...
            reply_queue.put({"status": "success", "message": f"Window {window_id} closed"})
...
class TakeBreakCommand(Command):
    def execute(self, overlay_manager, args, reply_queue):
        ...
        reply_queue.put({"status": "success", "message": f"Break started for {duration_seconds} seconds"})
...
class OverlayManager:
    ...
    def _init_window_and_pump(self):
        ...
        win32gui.SetLayeredWindowAttributes(
            self.hwnd, self._transparent_key, 200, win32con.LWA_COLORKEY | win32con.LWA_ALPHA
        )
    ...
    def start(self):
        if self._executor:
            return
        self._executor = ThreadPoolExecutor(max_workers=4)  # Increased to accommodate countdown manager
        self._executor.submit(self._init_window_and_pump)
        self._executor.submit(self._run_pipe_server)
        self._executor.submit(self._run_command_thread)
        self._executor.submit(self._run_countdown_manager)
        self._ready.wait()
    ...
    def shutdown(self, join_timeout: float = 5.0):
        ...
        if self._executor:
            self._executor.shutdown(wait=True, timeout=join_timeout)
    ...
    def add_highlight_window(self, left, top, right, bottom, duration_s):
        ...
        self.rectangles.append({"id": rid, "coords": (left, top, right, bottom), "color": color})
    ...
    def add_qrcode_window(self, metadata: str | dict, timeout_seconds: int, caption: str | None = None) -> int:
        ...
        threading.Timer(timeout_seconds, lambda: self.remove_qrcode_window(qr_id)).start()
        win32gui.InvalidateRect(self.hwnd, None, True)
        return qr_id
    ...
    def onPaint(self, hwnd):
        ...
        draw_all(hdc, full, self.rectangles, self.countdowns, self.qrcodes, self._transparent_key)
